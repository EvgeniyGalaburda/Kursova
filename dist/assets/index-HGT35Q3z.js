...
const Geolocation = registerPlugin("Geolocation", {
  web: () => __vitePreload(() => import("./web-CUsrngx2.js"), true ? [] : void 0).then((m) => new m.GeolocationWeb())
});
...
customElements.define("home-page", class extends HTMLElement {
  async mainWeather() {
    ...
    const mainWeather = weather.list.filter((_, i) => i % 8 == 0);
    return mainWeather;
  }
  async connectedCallback() {
    const weather = await getWeather();
    const mainWeather = await this.mainWeather();
    this.innerHTML = `
        <ion-content>
        <h1>${weather.city.name}</h1>
            <ul>
                ${mainWeather.map((day) => `<li>${day.main.temp}</li>`).join("")}
            </ul>
        </ion-content>
        `;
  }
